...
alias ..='cd ..'
alias ci='git commit'
alias csconf="gh cs ssh --config | perl -pe 's/^(Host cs\.[^.]*)\..*/\$1/' > ~/.config/ssh.d/codespaces"
alias dus='du -sh *| sort -h'
alias gg='git sw'
alias grl='git remote | xargs git remote show -n'
alias pssh='xargs -I@ -P"${P:-100}" -- ssh @ --'
alias sgit=git
...
then
    ...
    precmd() {
        {
            vcs_info
            PROMPT="$RCOLOR"'['"${HCOLOR}${CODESPACE_NAME:-%m}${GH_SITE:+.$GH_SITE}${RCOLOR}]${PROMPT_EXTRA}"'[%D{%f.%m}•%T]'"$PCHROOT${vcs_info_msg_0_}${VIRTUAL_ENV_PROMPT:+[}$VIRTUAL_ENV_PROMPT${VIRTUAL_ENV_PROMPT:+]}(%?)%(1j. |${jobtexts%% *}|.)%(!. #.❯)$CPNONE "      # ➤ • ❯
        } 2>/dev/null
    }
